...
**Vermes na parede:** mesma técnica de posição em coordenadas cilíndricas da v3 (ângulo + posição ao longo do eixo do túnel), mas **v4 trocou o corpo inteiro** depois de feedback direto: a corrente de cápsulas (`InstancedMesh` de `CapsuleGeometry`, uma por segmento) lia como "quadrado, de vídeo game" — o Gabriel mandou um vídeo real de microscopia de nematoide como referência (corpo único, liso, translúcido, afinando gradualmente nas duas pontas, sem nenhuma "junta" visível). A correção: em vez de encadear peças rígidas, o corpo virou **uma única `LatheGeometry`** (perfil fino→grosso→fino revolvido, feito pra imitar exatamente o afilamento do nematoide de referência — cauda mais longa/fina que a cabeça) **dobrada por vertex shader** — mesma família de técnica já usada no túnel e no Campo Lunar (geometria estática + deformação por GPU via função periódica de `uTime`), só que aqui a onda viaja ao longo do comprimento local do corpo (`z01 = position.z + 0.5`) com um envelope (`pow(z01,1.3)`) que faz a cauda chicotear mais que a cabeça — mais parecido com rastejo real. Continua `InstancedMesh` (1 draw call pra todos os vermes), mas agora **1 matriz por verme por frame, não 1 por segmento** — corte real de custo por frame, além de mais bonito. Sombreamento: fresnel simples (`pow(1-dot(normal,viewDir), 2.2)`) misturando cor de corpo com cor de borda, dando o aspecto "gelatinoso/translúcido" do vídeo de referência sem precisar de iluminação PBR completa. A opacidade "some" na limpeza via `smoothstep` direto no fragment shader (`uProgress` como uniform), não mais em JS por instância.

**Otimização mobile (pedido explícito, 28/08):** a versão de cápsulas fazia, por verme por segmento por frame, 2-3 alocações de `new THREE.Vector3()` — multiplicado por ~9 segmentos × ~20 vermes × 60fps, é pressão real de garbage collection, provável causa de engasgo em aparelho fraco. A reescrita pra "1 matriz por verme" elimina esse padrão inteiro (o objeto `dummy` é reaproveitado, nunca recriado). Outros cortes de mobile nessa mesma passada: malha do túnel `14×34` segmentos (era `20×48`), `pixelRatio` capado em `1.25` (era `1.5`), menos vermes (`10` vs `18`) e menos segmentos radiais no perfil do Lathe (`7` vs `10`) — tudo via a mesma flag `isMobile = innerWidth < 700` já em uso no resto do kit.

**v5 — modelo 3D real derivado do vídeo de referência (28/08, mesmo dia):** depois de ver a v4, o Gabriel deixou claro que "não mudou nada, os vermes foram os que eu fiz" — o pedido original era usar o vídeo de referência (footage real de microscopia de nematoide) como base do objeto 3D, não uma forma programada do zero, mesmo que inspirada nele. Fluxo: extraiu-se um frame do vídeo (`ffmpeg -i video.mp4 -vf fps=1 frame_%03d.png`; o `ffmpeg` não vinha instalado no sandbox, `apt-get update && apt-get install -y --no-install-recommends ffmpeg` resolveu — o índice de pacotes estava desatualizado, faltava só o `update` antes do `install`), subiu-se um frame limpo (verme inteiro visível, boa curva em S) via `media_upload`/`media_confirm`, e gerou-se um GLB com `generate_3d` (`model:"image_to_3d"`, `should_texture:true`, `symmetry_mode:"off"` — o corpo real não é simétrico —, `target_polycount:4000`, baixo porque são vários vermes pequenos de fundo).

**Integração com rede de segurança obrigatória:** o GLB carrega via `GLTFLoader` (addon que precisou voltar pro importmap — tinha sido removido quando só o túnel/Campo Lunar usavam só `three` core). Ao carregar: extrai o primeiro mesh da cena, recentraliza pela bounding box, descobre o eixo mais comprido (heurística — assume que é o "comprimento" do verme) e rotaciona pra alinhar em Z, escala pra comprimento unitário — mesma convenção de posicionamento na parede do túnel já usada pelo procedural, então os dois são intercambiáveis do ponto de vista do resto do código. **Mas**: não dá pra reaproveitar o shader de dobra por vértice num mesh de topologia desconhecida com segurança — o modelo real anima só por rotação leve (balanço) na matriz de instância, não por deformação de vértice como o procedural. Todo o carregamento fica num `try/catch` com timeout de 9s: qualquer falha (CDN fora do ar, GLB malformado, mesh vazio, timeout) cai automaticamente pro corpo procedural da v4, que continua no código como fallback — a cena nunca fica sem verme.

⚠️ **Limitação de verificação, registrar sempre que isso se repetir:** o CDN onde o Higgsfield hospeda os GLBs gerados (mesmo domínio cloudfront das imagens) está bloqueado no sandbox onde as LPs são testadas — não dá pra confirmar visualmente se a reconstrução 3D do vídeo ficou boa antes de publicar. O que FOI testado localmente: que o `try/catch` funciona de verdade (forçando a falha de rede real do sandbox, confirmado via Playwright que a cena cai pro procedural sem quebrar nada, zero erro de JS). O que NÃO foi testado: a qualidade da reconstrução 3D em si (reconstrução de objeto único e translúcido a partir de 1 imagem é um caso difícil pra a maioria dos modelos de image-to-3D, que assumem sujeito opaco). Pedir confirmação visual de quem estiver com acesso normal à internet antes de considerar essa parte fechada.
