...
2. **Tem que funcionar no Safari mobile ao vivo** (via raw.githack, sem servidor próprio): sempre incluir `html { overflow-x: hidden; }` e evitar qualquer recurso que dependa de HTTPS local/CORS especial. ⚠️ **Se a LP usar `position: sticky`** (ex.: cena WebGL "pinada" durante um trecho de rolagem — ver seção 7 do `REPERTORIO-TECNICO.md`): `overflow-x: hidden` sozinho no `html`/`body` força `overflow-y: auto` (regra da spec CSS quando só um eixo é setado), e isso quebra `position: sticky` nesses elementos — testado e confirmado em 28/08. Usar `overflow-x: hidden` seguido de `overflow-x: clip` na mesma declaração (a segunda linha sobrescreve a primeira em navegador que suporta `clip`; `hidden` fica de fallback pros raros que não suportam) resolve os dois problemas ao mesmo tempo.
